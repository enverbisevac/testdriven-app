...
class TestUserService(BaseTestCase):
    ...
    def test_users(self):
        """Ensure the /ping route behaves corectly."""
    ...
    def test_add_user(self):
        ...
        with self.client:
            ...
            self.assertIn('enver', data['message'])
            self.assertIn('success', data['status'])
    ...
    def test_add_user_invalid_keys(self):
        with self.client:
            response = self.client.post(
                '/users',
                data=json.dumps({'email': 'enver@bisevac.com'}),
                content_type='application/json'
            )
    ...
    def test_single_user(self):
        ...
        user = add_user(username='enver', email='enver@bisevac.com')
        with self.client:
            response = self.client.get(f'/users/{user.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('enver', data['data']['username'])
            self.assertIn('enver@bisevac.com', data['data']['email'])
            self.assertIn('success', data['status'])
    ...
    def test_single_user_incorect_id(self):
